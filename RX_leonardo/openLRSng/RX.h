...
//  Serial10.setBuffers(serial_rxbuffer, SERIAL_BUF_RX_SIZE, serial_txbuffer, SERIAL_BUF_TX_SIZE);
...
retry:
 ...
 //      Serial.println("Got packg");
    ...
    if ((bind_data.flags & TELEMETRY_MASK) == TELEMETRY_MAVLINK
      ...
      const uint8_t space = serial_space(TelemetrySerial.available(), SERIAL_BUF_RX_SIZE);
    ...
    if (bind_data.flags & TELEMETRY_MASK) {
      if ((tx_buf[0] ^ rx_buf[0]) & 0x40) {
        ...
        if((bind_data.flags & TELEMETRY_MASK) == TELEMETRY_MAVLINK) {
          uint8_t bytes = 0;
          while ((bytes < bind_data.serial_downlink - 1) && TelemetrySerial.available() > 0) {
            bytes++;
            const uint8_t ch = TelemetrySerial.read();
            tx_buf[bytes] = ch;
          }
          tx_buf[0] |= (0x3F & bytes);
        } else {
          if (TelemetrySerial.available() > 0) {
            ...
            while ((bytes < bind_data.serial_downlink - 1) && TelemetrySerial.available() > 0) {
              bytes++;
              tx_buf[bytes] = TelemetrySerial.read();
            }
            tx_buf[0] |= (0x37 & bytes);
          } else {
