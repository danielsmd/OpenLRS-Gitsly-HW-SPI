...
void myEEPROMwrite(int16_t addr, uint8_t data)
{
  ...
  while ((--retries) && (data != eeprom_read_byte((uint8_t *)addr))) {
    #ifdef DEBUG_DUMP_EEPROM    
      Serial.print("Read from ");
      Serial.print(addr);
      Serial.print(" Data: 0x");
      Serial.println(eeprom_read_byte(addr),HEX);
    #endif
...
start:
    ...
    for (uint8_t i = 0; i < dataSize; i++, addressNeedle++) {
      if (!write) {
        *((uint8_t*)dataAddress + i) = eeprom_read_byte((uint8_t *)(addressNeedle));
              ...
              Serial.print("Read from1 ");
              Serial.print(addressNeedle);
              Serial.print(" Data: 0x");
              Serial.println(eeprom_read_byte((uint8_t *)(addressNeedle)),HEX);
            #endif
      } else {
    ...
    if (!write) {
      CRC = eeprom_read_byte((uint8_t *)addressNeedle) << 8 | eeprom_read_byte((uint8_t *)(addressNeedle + 1));
            #ifdef DEBUG_DUMP_EEPROM
              Serial.print("CRC Read from ");
              Serial.print(addressNeedle);
              Serial.print(" and ");
              Serial.print(addressNeedle + 1);
              Serial.print(" CRC: 0x");
