...
uint8_t serial_okToSend; /* 0 = 
                            1 =
                            2 = if it is ok to send serial instead of servo
                            3 = Sent but no ACK
                            4 = Resend
                            */
...
void loop(void)
{
  ...
  if (RF_Mode == Received) {
    ...
    RF_Mode = Receive;
    spiSendAddress(0x7f); // Send the package read command
  //  Serial.print("New Data.Time:"); Serial.print(millis());Serial.print(" ");
    for (int16_t i = 0; i < bind_data.serial_downlink; i++) {
      rx_buf[i] = spiReadData();
    }
    /*
    for (int16_t i = 0; i < bind_data.serial_downlink; i++) {
      Serial.print(rx_buf[i]);Serial.print(" ");delay(1);
    }
    */
  ...
  if ((time - lastSent) >= getInterval(&bind_data)) {
    ...
    if ((ppmAge < 8) || (!TX_CONFIG_GETMINCH())) {
      ...
      if (lastTelemetry) {
       ...
       //   Serial.print ("Telemetry link re-established ");  //Tot timpul cand receptioneaza Tele apare
          buzzerOff();
        }
      }
      ...
      if (TelemetrySerial.available() > 0 && (serial_okToSend == 2)) {
        ...
        Serial.print ("Telemetry available:");
