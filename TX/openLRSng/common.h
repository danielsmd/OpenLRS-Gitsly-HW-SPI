...
// **** SPI bit banging functions
  ...
  void spiSendCommand(uint8_t command)    //HW_SPI
  {
    nSEL_on;
    nSEL_off;             
    //SPI.transfer(command & 0x80);         ///Orig
    SPI.transfer(command );         
  }
  
  void spiSendAddress(uint8_t i)     //HW_SPI
  {
    spiSendCommand(i & 0x7f);
  }
  
  void spiWriteData(uint8_t i)       //HW_SPI
  {
   //SPI.transfer( i & 0x80);     //orig
   SPI.transfer( i );
  }
  
  uint8_t spiReadData(void)      //HW_SPI
  {
    return SPI.transfer(0x00);
  }
  
  uint8_t spiReadRegister(uint8_t address)     //HW_SPI
  {
    uint8_t result;
    nSEL_off;
    spiSendAddress(address);
  ...
  void spiWriteRegister(uint8_t address, uint8_t data)     //HW_SPI
  {
    nSEL_off;
    address |= 0x80; 
